...
`r bind_trash_df |>  filter(data == "gwynns", year == 2022, month == "June") |>  summarise(cignum = sum(cigarette_butts, na.rm = TRUE)) |> pull(cignum)` butts.
...
* There are `r nrow(zip_zori_df)` observations and `r ncol(zip_zori_df)` variables. There are key variables, such as `county`, `zip_code`, `neighborhood`, `date`, and `rental_price`. We are able to see 
* In `zip_zori_df`, there are `r zip_zori_df |> summarise(unique_zips = n_distinct(zip_code)) |> pull(unique_zips)` unique zip codes and `r zip_zori_df |> summarise(unique_neigh = n_distinct(neighborhood)) |>  pull(unique_neigh)` unique neighborhoods.
